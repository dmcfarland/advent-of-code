...
fn main() {
    ...
    let part2_result = part2(&input, 70, 1024);
...
fn part2(input: &str, grid_size: usize, num_bytes: usize) -> (usize, usize) {
    let mut grid = vec![vec!['.'; grid_size + 1]; grid_size + 1];
    let re = Regex::new(r"(\d*),(\d*)").unwrap();
    let bad_bytes: Vec<(usize, usize)> = re
        .captures_iter(input)
    ...
    for i in num_bytes..bad_bytes.len() {
        let mut cur = vec![(0, 0)];
        let b = bad_bytes[i];
        println!("BadByte: {} {:?}", i, b);
        grid[b.1][b.0] = '#';
        ...
        let mut prev: Vec<(usize, usize)> = Vec::new();
        while let Some(&last) = cur.last() {
            if last == (grid_size, grid_size) || cur.is_empty() {
                break;
            }
        ...
        if !prev.contains(&(grid_size, grid_size)) {
            return (b.0, b.1);
        }
    }
